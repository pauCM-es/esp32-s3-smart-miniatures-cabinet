#include "SdSettingsStore.h"

#include "SettingsJson.h"

SdSettingsStore::SdSettingsStore(fs::FS& fs, const char* path)
    : fs_(fs), path_(path) {}

bool SdSettingsStore::load(CabinetSettings& settings) {
...
bool SdSettingsStore::save(const CabinetSettings& settings) {
    ...
    File tempFile = fs_.open(tempPath.c_str(), FILE_WRITE);
    if (!tempFile) {
        return false;
    }
...
bool SdSettingsStore::ensureParentDirectory() {
    const int separator = path_.lastIndexOf('/');
    if (separator <= 0) {
        return true;
    }

    const String directory = path_.substring(0, separator);
