...
const Y = (e, t) => e.shadowRoot.querySelector(t)?.value, Ht = (e) => ({
  ...
  const t = e._miniatures, i = t.find((r) => r.id === e._editingMiniId), a = ct(t, e._catalogueSort);
  return l`<div class="miniatures-grid">
		...
		<section class="panel-card mini-list-card">
			...
			${dt(
    e._catalogueSort,
    (r) => e.actions.setSort("_catalogueSort", r)
  )}
			<div class="mini-list">
				${a.map(
    (r) => l`<div class="mini-row">
							${B(r.name)}
							<div class="mini-main">
								<b>${r.name}</b
								><span
									>${r.collection || "No collection"}</span
								>
							</div>
							...
							<div
								class="position-badge ${r.shelf ? "" : "unassigned"}">
								${r.shelf ? `S${r.shelf} · L${r.location}` : "Unassigned"}
							</div>
							<div class="row-actions">
								<button
									class="ghost"
									@click=${() => e.actions.editMini(r.id)}>
									Edit</button
								><button
									class="danger ghost"
									@click=${() => e.actions.deleteMini(r.id)}>
									Delete
								</button>
							</div>
						</div>`
